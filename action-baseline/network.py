...
class Network(nn.Module):
    ...
    def _image_to_head(self, pretrained='Kinetics'):
        if pretrained == 'ImageNet':
            ...
            net_conv = self.i3d._head_feature(self._image)

        return net_conv[:, :, 7, :, :], net_conv

    def _image_to_tail(self, x, pretrained='Kinetics'):
        if pretrained == 'Kinetics':
            return self.i3d._tail_feature(x)
    ...
    def _init_modules(self, pretrained='Kinetics'):
        ...
        # classification
        self.cls_score_net = nn.Linear(1024, self._num_classes)
        self.bbox_pred_net = nn.Linear(1024, self._num_classes * 4)
    ...
    def _predict(self, pretrained='Kinetics'):
        ...
        if not self._check:
            if cfg.POOLING_MODE == 'align':
                pool5 = torch.cat([self._roi_align_layer(video_feature[:, :, idx, :, :], rois).unsqueeze(dim=2) for idx in range(16)], dim=2)
            else:
                pool5 = torch.cat([self._roi_pool_layer(video_feature[:, :, idx, :, :], rois).unsqueeze(dim=2) for idx in range(16)], dim=2)

            if self._mode == 'TRAIN':
                torch.backends.cudnn.benchmark = True
            tail_out = self._image_to_tail(pool5)
            tail_out = F.avg_pool3d(tail_out, (8, 4, 4))
            tail_out = tail_out.view(-1, 1024)

            cls_prob, bbox_pred = self._region_classification(tail_out)
    ...
    def train_step(self, blobs, train_op):
        self._check = False
        self.forward(blobs['data'], blobs['im_info'], blobs['boxes'], blobs['gt_classes'])
        if not self._check:
            ...
            # return rpn_loss_cls, rpn_loss_box, 0, 0, loss, None
    ...
    def _add_losses(self, sigma_rpn=3.0):
        ...
        cross_entropy = F.binary_cross_entropy_with_logits(cls_score, label.float(), weight=cls_inside_weights.float(), reduction='sum')
        cross_entropy /= float(avg_factor)
