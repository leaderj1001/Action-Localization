...
from nets.bbox_transform import bbox_transform, bbox_overlaps

import torch


def proposal_target_layer(rpn_rois, rpn_scores, gt_boxes, gt_classes, _num_classes):
    """
    ...
    if cfg.TRAIN.USE_GT:
        ...
        all_rois = torch.cat((all_rois, torch.cat((zeros, gt_boxes[:, :]), 1)), 0)
    ...
    labels, rois, roi_scores, bbox_targets, bbox_inside_weights, cls_inside_weights = _sample_rois(
        all_rois, all_scores, gt_boxes, gt_classes, fg_rois_per_image, rois_per_image, _num_classes)
    ...
    return rois, roi_scores, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights, cls_inside_weights
...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
    ...
    clss = bbox_target_data[:, 0:81]
    clss = clss.sum(dim=-1)
    bbox_targets = clss.new_zeros(clss.numel(), 4 * num_classes)
    bbox_inside_weights = clss.new_zeros(bbox_targets.shape)
    cls_inside_weights = clss.new_zeros(clss.numel(), 1)
    inds = (clss > 0).nonzero().view(-1)

    if inds.numel() > 0:
        out_inds = torch.arange(inds[-1].item(), inds[-1].item() + inds.numel() // 2)
        cls_inside_weights[inds] = 1
        cls_inside_weights[out_inds] = 1
    ...
    if inds.numel() > 0:
        ...
        bbox_targets[dim1_inds, dim2_inds] = bbox_target_data[inds][:, 81:]
        bbox_inside_weights[dim1_inds, dim2_inds] = bbox_targets.new(cfg.TRAIN.BBOX_INSIDE_WEIGHTS).view(-1, 4).expand_as(dim1_inds)

    return bbox_targets, bbox_inside_weights, cls_inside_weights
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    return torch.cat([labels, targets], 1)


def _sample_rois(all_rois, all_scores, gt_boxes, gt_classes, fg_rois_per_image, rois_per_image, num_classes):
    """Generate a random sample of RoIs comprising foreground and background examples."""

    overlaps = bbox_overlaps(all_rois[:, 1:5].data, gt_boxes.data)
    ...
    bbox_target_data = _compute_targets(rois[:, 1:5].data, gt_boxes[gt_assignment[keep_inds]][:, :].data, labels.data)

    bbox_targets, bbox_inside_weights, cls_inside_weights = _get_bbox_regression_labels(bbox_target_data, num_classes)

    return labels, rois, roi_scores, bbox_targets, bbox_inside_weights, cls_inside_weights
