...
# Heatmap indices to find each limb (joint connection). Eg: limb_type=1 is Neck->LShoulder, so joint_to_limb_heatmap_relationship[1] represents the indices of heatmaps to look for joints: neck=1, LShoulder=5
...
# Eg: limb_type=1 is Neck->LShoulder, so PAFneckLShoulder_x=paf_xy_coords_per_limb[1][0] and PAFneckLShoulder_y=paf_xy_coords_per_limb[1][1]
...
# Color code used to plot different joints and limbs (eg: joint_type=3 and limb_type=3 will use colors[3])
colors = [
...
def compute_resized_coords(coords, resizeFactor):
    """
    Given the index/coordinates of a cell in some input array (e.g. image), provides the new coordinates if that array was resized by making it resizeFactor times bigger.
    E.g.: image of size 3x3 is resized to 6x6 (resizeFactor=2), we'd like to know the new coordinates of cell [1,2] -> Function would return [2.5,4.5]
    :param coords: Coordinates (indices) of a cell in some input array
    :param resizeFactor: Resize coefficient = shape_dest/shape_source. E.g.: resizeFactor=2 means the destination array is twice as big as the original one
    :return: Coordinates in an array of size shape_dest=resizeFactor*shape_source, expressing the array indices of the closest point to 'coords' if an image of size shape_source was resized to shape_dest
    """

    # 1) Add 0.5 to coords to get coordinates of center of the pixel (e.g. index [0,0] represents the pixel at location [0.5,0.5])
...
def plot_pose(img_orig, joint_list, person_to_joint_assoc, bool_fast_plot=True, plot_ear_to_shoulder=False):
    ...
    for limb_type in range(which_limbs_to_plot):
        for person_joint_info in person_to_joint_assoc:
            ...
            # For faster plotting, just plot over canvas instead of constantly copying it
...
def decode_pose(img_orig, param, heatmaps, pafs):
    ...
    # joint_list is an unravel'd version of joint_list_per_joint, where we add a 5th column to indicate the joint_type (0=nose, 1=neck...)
